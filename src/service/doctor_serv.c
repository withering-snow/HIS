...
Status Serv_doc_admission(long long ward_id, int bed_label, long long deposit){
    ...
    // 创建入院记录
    Record_T r = Rec_admit_new(0, _cur_doc_pat.pat_id, ward_id, bed_label, deposit);
    List_push_back(Data_get_record(), &r);
...
Status Serv_doc_discharge(long long pat_id){
    ...
    if(fund != NULL){
        ...
        paid = (balance >= total_bill) ? total_bill : balance;
        Fund_withdraw(fund, paid);
    }
    ...
    // 创建出院记录
    Record_T r = Rec_disc_new(0, pat_id, total_bill, paid);
    List_push_back(Data_get_record(), &r);

    Log_printf(CLASS_DOCTOR, _cur_doc_pat.doc_id, "医生办理病人[%lld]出院 账单[%lld]实付[%lld]", pat_id, total_bill, paid);
    return HIS_OK;
}
